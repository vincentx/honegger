...
(function($) {
  ...
  ComponentEditor = (function() {
    ...
    return {
      ...
      "new": function(target, component, config, value) {
        var editor;
        if (value == null) {
  ...
  ContentComponent = function(api, spi) {
    ...
    createComponentEditor = function(target, name, id, config, value) {
      return newComponent(ComponentEditor["new"](target, components[name], config, value), id, name, config);
    };
    createComponentControl = function(target, name, id, config, value) {
      return newComponent(components[name].control(target, value), id, name, config).data('component-content', value);
    };
    createPlaceHolder = function(target, name, id, config, value) {
      var placeholder;
      placeholder = components[name].placeholder != null ? components[name].placeholder(target) : $('<div></div>');
      return newComponent(placeholder, id, name, config).data('component-content', value);
    };
    createComponent = function(components, creator) {
      return components().each(function() {
        var component;
        component = $(this);
        return component.replaceWith(creator(component, component.data('component-type'), component.data('component-id'), ComponentEditor.getConfiguration(component), ComponentEditor.getContent(component)));
      });
    };
    ...
    return {
      extensionPoints: function() {
        ...
        spi.insertComponent = function(target, name, config) {
          ...
          return target.append(createComponentEditor(null, name, IdGenerator.next(name), config));
        };
        ...
        spi.getComponentConfiguration = function(component) {
          ...
          if (componentType.getConfig != null) {
            return componentType.getConfig(component);
          } else {
        ...
        spi.getComponentContent = function(component) {
          var componentType;
          componentType = components[component.data('component-type')];
          if (componentType.getContent != null) {
            return componentType.getContent(component);
          } else {
...
(function($) {
  ...
  $.fn.honegger.page = function(options) {
    var initComponentContainer, removeControls;
    ...
    return {
      ...
      getConfig: function(page) {
        var config;
        config = {};
        options.spi.components(page).each(function() {
          var component;
          component = $(this);
          return config[component.attr('data-component-id')] = options.spi.getComponentConfiguration(component);
        });
        return config;
      },
      getContent: function(page) {
        ...
        options.spi.components(page).each(function() {
          var component;
          component = $(this);
          return content[component.attr('data-component-id')] = options.spi.getComponentContent(component);
        });
        return content;
      }
    };
...
(function($) {
  ...
  ContentTemplate = function(api, spi) {
    return {
      extensionPoints: function() {
        return api.getContentTemplate = function() {
          ...
          spi.components(composer).each(function() {
            ...
            return content[component.data('component-id')] = spi.getComponentContent(component);
          });
          return {
