(function($, document, window) {
  ...
  Honegger = function(element, options) {
    var bindHotkey, composer, currentRange, disable, disabled, enable, execCommand, getConfiguration, init, insideComposer, installed, makeComposer, makeComposers, mode, notInsideComponent, self, toolbar;
    self = this;
    ...
    this.insertComponent = function(name) {
      var range;
      if (!(!disabled && (installed.get(name) != null) && (range = currentRange()))) {
        return;
      }
      return range.insertNode(installed.newComponent(name)[0]);
    };
    ...
    this.changeMode = function(new_mode) {
      ...
      if (mode === 'edit') {
        ...
        return composer.find(options.componentSelector).each(function() {
          return installed.modes[mode]($(this));
        });
      } else {
        self.disable();
        return composer.find(options.componentSelector).each(function() {
    ...
    this.getTemplate = function(handler) {
      ...
      return installed.getTemplate(template, function(dataTemplate, configurations) {
  ...
  $.fn.honegger.components = function(composer, options) {
    var changeMode, componentIds, components, generateId, getConfiguration, setConfiguration;
    components = {};
    componentIds = {};
    ...
    return {
      ...
      newComponent: function(name) {
        return components[name].editor(options.componentEditorTemplate, {}).data('component-config', {}).attr('data-component-type', name).attr('data-role', 'component').attr('data-component-id', generateId(name));
      },
      modes: {
      ...
      getTemplate: function(template, handler) {
        var configurations, dataTemplate;
        dataTemplate = {};
        configurations = {};
        ...
        return handler(dataTemplate, configurations);
      },
      get: function(name) {
