(function($, document, window) {
  ...
  Honegger = function(element, options) {
    var bindHotkey, changeMode, components, composer, currentRange, disable, disabled, editMode, enable, execCommand, getConfiguration, init, insideComposer, installed, makeComposer, makeComposers, mode, notInsideComponent, previewMode, self, toolbar;
    self = this;
    components = {};
    ...
    this.insertComponent = function(name) {
      ...
      editor = components[name].editor(options.componentEditorTemplate, {}).data('component-config', {}).attr('data-component-type', name).attr('data-role', 'component').attr('data-component-id', installed.generateId(name));
