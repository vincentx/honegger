...
(function($) {
  ...
  ComponentEditor = (function() {
    var ensureExist, getValue, setValue;
    ...
    return {
      getConfiguration: function(editor) {
        ...
        $('*[data-component-config-key]', editor).each(function() {
          ...
          return eval("config." + key + " = getValue(configElement)");
        });
        return config;
      },
      setConfiguration: function(editor, config) {
        $('*[data-component-config-key]', editor).each(function() {
          ...
          if (value != null) {
            return setValue(configElement, value);
          }
        });
        return editor;
      },
      setContent: function(editor, content) {
        $('*[name]', editor).each(function() {
          var contentElement, value;
          contentElement = $(this);
          value = eval("content." + (contentElement.attr('name')));
          if (value != null) {
            return setValue(contentElement, value);
          }
        });
        return editor;
      },
      "new": function(component, config) {
        var editor;
        editor = this.setConfiguration(component.editor("template", config), config);
        if (component.dataTemplate) {
          this.setContent(editor, component.dataTemplate);
        }
        return editor;
      }
    };
  ...
  ContentComponent = function(api, spi) {
    ...
    createComponentEditor = function(name, id, config) {
      return newComponent(ComponentEditor["new"](components[name], config), id, name, config);
    };
