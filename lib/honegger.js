  ...
  return $.fn.honegger.components = function(composer, options) {
    ...
    getConfiguration = function(element) {
      ...
      $(options.configurationSelector, element).each(function() {
        ...
        if (key.indexOf('.') !== -1) {
          ...
          return eval("config." + key + " = configElement.val()");
        } else {
          return config[key] = configElement.val();
        }
      });
      return config;
    };
    ...
    return {
      ...
      newComponent: function(name, component_config) {
        return setConfiguration(config(components[name].editor(options.componentEditorTemplate, component_config), generateId(name), name, component_config), component_config);
      },
      modes: {
